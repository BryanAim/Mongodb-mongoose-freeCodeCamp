...
var createAndSavePerson = function(done) {
  ...
  let isaleBryan = new Person({
    name: 'Isale Bryan',
    age: 22,
    favoriteFoods: ['pizza', 'meat']
  });
  isaleBryan.save(function(err, data) {
...
let arrayOfPeople = [{
  name: 'James King',
  age: 34,
  favoriteFoods: ['who cares', 'not me']
},{
  name: 'Judas Christian',
  age: 54,
  favoriteFoods: ['silver', 'coins']
}]
...
var findPeopleByName = function(personName, done) {
  Person.find({name: personName}, function (err, personDocument) {
    if (err) {
      return console.error(err)
    } else {
      return done(null, personDocument)
    }
  })
...
var findOneByFood = function(food, done) {
  Person.findOne({favoriteFoods: food}, function (err, personDoc) { 
    if (err) {
      return console.error(err);
    } else {
      return done(null, personDoc)
    }
   })
